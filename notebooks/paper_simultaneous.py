...
targets = targets[targets['Type']=='M Dwarf'] # resetrict it to m dwarfs
targets = targets[targets['TESS?'] != 'N']
names = targets['Name']
print(targets.keys())
...
for j in range(len(names)):
    ...
    if '%ssimultaneous_%s_.pdf' % (savedir,name.replace(' ','_').lower()) in saved_files:
        print('\n\nAlready done',name)
        continue
    ...
    for j in range(len(sects)):
        avg_preds.append(Table.read('../results/final/avg_preds_%s_%d.csv' % (name.replace(' ','_').lower(), j))['avg_preds'].data)
    ...
    if name in stars_simultaneous.keys():
        ...
        plt.savefig(figname,bbox_inches='tight')
        figname = '%ssimultaneous_%s.pdf' % (savedir,name.replace(' ','_').lower())
